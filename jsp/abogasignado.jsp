...
ResultSet  rss = conn.consultar("select * from estu_asig_temp where periodo='"+periodo+"' order by nombre");
...
if (!chequearOpcion()) {	
		alert("Debe seleccionar un Estudiante para continuar.");
	}else{
		frm.action="vercasosasignados.jsp";
		frm.submit();
		}
}
function volver(frm){
		frm.action="abogadoasignado.jsp";
		frm.submit();		
}
function modificar2(frm){	
		frm.action="abogasignado.jsp";
		frm.submit();		
}
...
<%}%></td><td bgcolor="#ffffff" valign="top" width="687" align="left"> 
 <div id="pagina_contenido" style=" padding: 0 0 0 15px;">
  <div class="actualizacion">Administrador - Consultorio</div>
  <h1>Abogados en Formacion Asignados   / Periodo:  <%=periodo%></h1>
  <p></p><br/>
...
<input name="periodo"  type="hidden"  value="<%=periodo%>" >
    <div><strong class="text_negro">BUSCAR ESTUDIANTE: </strong> <input name="buscar"  type="text"  class="text_file" size="30" onChange="javascript:while(''+this.value.charAt(0)==' ')this.value=this.value.substring(1,this.value.length);this.value=this.value.toUpperCase();" value="<%if(buscar=="null" || buscar==""){}else{ out.print(buscar);}%>">
                  <input name="Input3" type="button" onClick="javascript:modificar2(document.forms['form1'])" class="botones" value="  Buscar  " size="100"> <input name="submit2" type="reset" class="botones" value=" Limpiar " onClick="javascript:cargar(document.forms['form1']);"/></div> 
  <br/>
		<div align="right" > 		 <%if(contt!=0){%>
				  <input name="input" type="button" class="botones" onClick="javascript:exportar(document.forms['form1'])"  value=" Exportar a Excel " /></div>
                  <%}%>
    <table width="650" border="0" align="center" cellpadding="1" cellspacing="0" class="fondo_tabla">
                  <tr>
                    <td width="650" height="17" valign="top">
                      <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="ffffff">
                        ...
                        <tr class="fondo_celda_2">
                          ...
                          <td colspan="5" align="center" class="text_negro">Casos Asignados</td>
                          </tr>
                        <tr class="fondo_celda_2">
                          <td height="20" colspan="4"  align="center" class="text_negro">Casos Asignados Por Area </td>
						...
						if(buscar=="null" || buscar==""){  				
							consulta="select  * from estu_asig_temp where periodo='"+periodo+"' order by nombre limit 20 offset "+dep+"";
						}else{							
							consulta= " select  * from estu_asig_temp where (periodo='"+periodo+"' and upper(identificacion) like '%"+buscar+"%') OR (periodo='"+periodo+"' and upper(nombre) like '%"+buscar+"%')  or (periodo='"+periodo+"' and upper(apellido) like '%"+buscar+"%') order by nombre";
						}								
					 ResultSet  rs=con.consultar(consulta);
int cont = con.contar(rs);String[][] opes = new String[cont][10];
rs.beforeFirst();
						if(cont!=0){ 
							rs.next();
							for(int ii=0;ii<cont;ii++)
								{ 
									...
									opes[ii][4]=rs.getString("laboral");
									opes[ii][5]=rs.getString("penal");
									opes[ii][6]=rs.getString("privada");
									opes[ii][7]=rs.getString("publica");
									opes[ii][8]=rs.getString("casos");
									rs.next();
							}	
 ...
 <img src="img/system-search.png"  width="16" height="16"  title="Ver Radicado" style="cursor:pointer"/>
                          ...
                          <input type="hidden" name="nombre" value="<%= opes[ii][2] %> <%= opes[ii][3] %>"/>
						  <% 
								int contar;
								int contar2;
								int contar3;
							    ...
							    total=total+contar5;
								contar=Integer.parseInt(opes[ii][4]);
								la=la+contar;
								contar2=Integer.parseInt(opes[ii][5]);
								pe=pe+contar2;
								contar3=Integer.parseInt(opes[ii][6]);
								pr=pr+contar3;
								contar4=Integer.parseInt(opes[ii][7]);
								pu=pu+contar4;  %>
